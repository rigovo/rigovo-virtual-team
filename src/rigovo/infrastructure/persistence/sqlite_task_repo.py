...
class SqliteTaskRepository(TaskRepository):
    ...
    def _step_to_dict(step: PipelineStep) -> dict:
        return {
            ...
            "cache_saved_tokens": step.cache_saved_tokens,
            "cache_saved_cost_usd": step.cache_saved_cost_usd,
            ...
            "execution_verified": step.execution_verified,
        }
        ...
        try:
            ...
            task.pipeline_steps = [
                PipelineStep(
                    ...
                    cache_source=s.get("cache_source", "none"),
                    cache_saved_tokens=s.get("cache_saved_tokens", 0),
                    cache_saved_cost_usd=s.get("cache_saved_cost_usd", 0.0),
                    gate_passed=s.get("gate_passed"),
                    gate_score=s.get("gate_score"),
                    retry_count=s.get("retry_count", 0),
                    gate_violations=s.get("gate_violations", []),
                    execution_log=s.get("execution_log", []),
                    execution_verified=s.get("execution_verified", False),
                )
                for s in raw_steps
            ]
