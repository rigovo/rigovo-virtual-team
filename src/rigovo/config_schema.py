...
def rigovo_yml_to_string(config: RigovoConfig) -> str:
    """Return a complete, human-readable rigovo.yml string for *config* WITHOUT
    writing anything to disk.  Used by the Settings API to return a fully-
    normalised YAML that always includes every section (even if the on-disk
    file was created before a section was added to the schema).
    """
