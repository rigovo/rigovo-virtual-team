...
const ROLES: Record<string, RoleMeta> = {
  master: {
    ...
    color: "role-text-emerald",
    surface: "role-surface-emerald",
    border: "role-border-emerald",
  },
  planner: {
    icon: "\uD83E\uDDE0",
    label: "Project Manager",
    color: "role-text-violet",
    surface: "role-surface-violet",
    border: "role-border-violet",
  },
  lead: {
    icon: "\uD83C\uDFAF",
    label: "Tech Lead",
    color: "role-text-purple",
    surface: "role-surface-purple",
    border: "role-border-purple",
  },
  coder: {
    icon: "\uD83D\uDCBB",
    label: "Software Engineer",
    color: "role-text-sky",
    surface: "role-surface-sky",
    border: "role-border-sky",
  },
  reviewer: {
    icon: "\uD83D\uDD0E",
    label: "Code Reviewer",
  ...
  qa: {
    icon: "\uD83E\uDDEA",
    label: "QA Engineer",
    color: "role-text-amber",
    surface: "role-surface-amber",
    border: "role-border-amber",
  },
  security: {
    icon: "\uD83D\uDD10",
    label: "Security Engineer",
    color: "role-text-rose",
    surface: "role-surface-rose",
    border: "role-border-rose",
  },
  devops: {
    icon: "\u2699\uFE0F",
    label: "DevOps Engineer",
    color: "role-text-indigo",
    surface: "role-surface-indigo",
    border: "role-border-indigo",
  },
  sre: {
    icon: "\uD83D\uDCC8",
    label: "SRE Engineer",
    color: "role-text-cyan",
    surface: "role-surface-cyan",
    border: "role-border-cyan",
  },
  docs: {
    icon: "\uD83D\uDCDD",
...
function statusTone(status: TaskStep["status"]): string {
  if (status === "complete") return "role-text-indigo";
  if (status === "running") return "role-text-indigo";
...
function eventKind(step: TaskStep): NarrativeKind {
  ...
  if (REVIEW_ROLES.has((step.agent_role || step.agent).toLowerCase()))
    return "challenge";
  return "action";
}
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${
        kind === "challenge"
          ? "role-badge-rose role-border-rose"
          : "role-badge-indigo role-border-indigo"
      }`}
    >
      {kind}
    </span>
...
function DecisionCard({
  ...
  return (
    <div
      className="narrative-decision animate-fadeup"
    ...
    >
      ...
      <p className="mt-1 text-[12px] text-[var(--ui-text-secondary)]">
        {first ? (
          <>
            Routed objective to{" "}
            <span className={target.color}>{target.label}</span> as starting
            owner.
          </>
        ) : (
          <>
            Handoff from <span className={source?.color}>{source?.label}</span>{" "}
            to <span className={target.color}>{target.label}</span> with context
            and evidence.
          </>
        )}
      </p>
...
function CollabRow({
  ...
  switch (event.type) {
    ...
    case "debate_round":
      ...
      bodyText = event.reviewer_feedback
        ? String(event.reviewer_feedback).slice(0, 220)
        : "";
      break;
  ...
  return (
    <div
      className={`feed-collab animate-fadeup ${contentClass}`}
      style={{ animationDelay: `${index * 40}ms` }}
    >
      <span className="text-[12px] flex-shrink-0 leading-none mt-0.5">
        {icon}
      </span>
...
function ReplanCard({
  ...
  const triggerReason = String(
    metadata.trigger_reason || "policy_replan",
  ).replace(/_/g, " ");
  ...
  const reason =
    event.summary ||
    event.action ||
    "Pipeline re-evaluated after reviewing outputs and gate results.";

  return (
    <div
      className="narrative-replan animate-fadeup"
      style={{ animationDelay: `${index * 40}ms` }}
    >
      <div className="flex items-center gap-2">
        <span className="text-sm">{isFailed ? "⚠️" : "🔄"}</span>
        <span
          className="narrative-meta-label"
          style={{ color: isFailed ? "var(--s-failed)" : "var(--s-waiting)" }}
        >
      ...
      <p className="mt-1.5 text-[12px] leading-relaxed text-[var(--ui-text-secondary)]">
        {reason}
      </p>
      ...
      {isTriggered && (
        <div className="mt-2 space-y-1 text-[11px] text-[var(--ui-text-muted)]">
          <div>
            <span className="font-semibold">Trigger:</span> {triggerReason}
          </div>
          <div>
          ...
          {targetRole && (
            <div>
              <span className="font-semibold">Retry:</span> {targetRole}
            </div>
          )}
          <div>
            <span className="font-semibold">Attempt:</span> {replanCount} of{" "}
            {maxReplans}
          </div>
        </div>
      )}
      ...
      {isFailed && (
        <div className="mt-2 space-y-1 text-[11px] text-[var(--ui-text-muted)]">
          <div>
            <span className="font-semibold">Reason:</span> {triggerReason}
          </div>
          <div>
            <span className="font-semibold">Max replans:</span> {maxReplans}{" "}
            exhausted
          </div>
        </div>
      )}
...
function MemoryInjectionCard({
  ...
  return (
    <div
      className="feed-memory animate-fadeup"
      style={{ animationDelay: `${index * 40}ms` }}
    >
      <div className="flex items-center gap-2">
        <span className="text-sm">📚</span>
        <span className="feed-memory-title">
          {event.role || "Agent"} received memory context
        </span>
        <span className="feed-memory-count">
              ...
              {mem.relevance_score !== undefined && (
                <span
                  className={`memory-relevance ${
                    mem.relevance_score >= 0.8
                      ? "high"
                      : mem.relevance_score >= 0.6
                        ? "medium"
                        : "low"
                  }`}
                >
...
function GovRow({ event, index }: { event: GovTimelineEvent; index: number }) {
  const isReplan =
    (event.action ?? "").toLowerCase().includes("replan") ||
  ...
  return (
    <div
      className="feed-gov-row animate-fadeup"
...
function EventCard({
  ...
  const hasCache =
    (step.cache_source && step.cache_source !== "none") ||
    ...
    >
      <header className="narrative-event-head">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span
            className={`text-[10px] font-medium uppercase ${statusTone(step.status)}`}
          >
            {step.status}
          </span>
      ...
      <div className="mt-3">
        {running && !step.output ? (
          ...
          <p className="text-[12px] text-[var(--ui-text-muted)] italic">
            {failed
              ? "Execution stopped before output was produced."
              : "Step completed without textual output."}
          </p>
        )}
      </div>
      ...
      {hasFiles && (
        <section className="narrative-files">
          <p className="narrative-meta-label">
            Evidence: files changed ({step.files_changed.length})
          </p>
          <div className="mt-1.5 flex flex-wrap gap-1">
            ...
            {step.files_changed.length > 6 && (
              <span className="narrative-file-pill">
                +{step.files_changed.length - 6} more
              </span>
            )}
          </div>
        </section>
      ...
      {showPills && (
        <div className="feed-pills">
          {hasFiles && (
            <button
              type="button"
              className="feed-pill files"
              onClick={onOpenFiles}
            >
              📁 {step.files_changed.length} file
              {step.files_changed.length !== 1 ? "s" : ""}
            </button>
          )}
          {gates.length > 0 && (
            <span
              className={`feed-pill ${gatesFailed ? "fail" : gatesPassed ? "pass" : ""}`}
            >
      ...
      {(tokenStats ||
        ...
        <div className="feed-pills mt-2">
          {tokenStats && tokenStats.total > 0 && (
            <>
              <span className="feed-pill">
                in {tokenStats.input.toLocaleString()}
              </span>
              <span className="feed-pill">
                out {tokenStats.output.toLocaleString()}
              </span>
              <span className="feed-pill">
                total {tokenStats.total.toLocaleString()}
              </span>
            </>
          )}
          {(retryCount ?? 0) > 0 && (
            <span className="feed-pill fail">retry {retryCount}</span>
          )}
          {(consultCount ?? 0) > 0 && (
            <span className="feed-pill">consult {consultCount}</span>
          )}
          {(debateCount ?? 0) > 0 && (
            <span className="feed-pill">debate {debateCount}</span>
          )}
          {(step.cache_saved_tokens ?? 0) > 0 && (
            <span className="feed-pill pass">
              saved {(step.cache_saved_tokens ?? 0).toLocaleString()} tok
            </span>
          )}
          {(step.cache_saved_cost_usd ?? 0) > 0 && (
            <span className="feed-pill pass">
              saved ${Number(step.cache_saved_cost_usd ?? 0).toFixed(4)}
            </span>
          )}
        </div>
      )}
...
function ResolutionCard({ step, index }: { step: TaskStep; index: number }) {
  ...
  const totalViolations = gates.reduce(
    (sum, g) => sum + (g.violation_count ?? 0),
    0,
  );
  ...
  const grouped = new Map<
    string,
    { gate: TaskStep["gate_results"][number]; count: number }
  >();
          ...
          return (
            <span
              ...
              className={`rounded-md px-2 py-0.5 text-[10px] ${
                g.passed ? "role-badge-indigo" : "role-badge-rose"
              }`}
            >
              {label}
              {count > 1 && <span className="ml-1 opacity-75">x{count}</span>}
              {g.violation_count != null &&
                g.violation_count > 0 &&
                !g.passed && (
  ...
  return (
    <section className="narrative-kickoff animate-fadeup">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold role-text-indigo">
          Master Brain
        </span>
        <span className="text-[10px] text-[var(--ui-text-muted)] uppercase">
          Orchestrator
        </span>
      </div>
      <p className="mt-2 text-[13px] text-[var(--ui-text-secondary)] leading-relaxed">
        Task classified as{" "}
        <span className="role-text-indigo font-semibold">
          {taskType || "engineering"}
        </span>
        . Building and supervising a {count}-agent virtual team for this run.
      </p>
    </section>
  );
}
...
export default function AgentTimeline({
    ...
    () =>
      steps.filter(
        (s) =>
          s.status !== "pending" ||
          Boolean(s.output) ||
          Boolean(s.started_at) ||
          Boolean(s.completed_at),
      ),
    [steps],
  );
  const queued = useMemo(
    () => steps.filter((s) => !visibleSteps.includes(s)),
    [steps, visibleSteps],
  );
  const costByRole = useMemo(() => {
    ...
    for (const entry of entries) {
      const role = String(entry.agent_role || "")
        .trim()
        .toLowerCase();
      if (!role) continue;
  ...
  function eventsForStep(step: TaskStep) {
    ...
    if (start === 0 && end === 0)
      return {
        collabEvents: [] as CollabEvent[],
        govEvents: [] as GovTimelineEvent[],
      };
  ...
  const stepWindows = visibleSteps.map((s) => ({
    start: toMs(s.started_at),
    end: toMs(s.completed_at),
  }));
  ...
  (collab?.events ?? []).forEach((e, i) => {
    ...
    if (
      ts > 0 &&
      stepWindows.some(
        ({ start, end }) => ts >= start && (end === 0 || ts <= end + 5000),
      )
    ) {
      usedCollabIds.add(i);
    }
  ...
  const orphanCollab = (collab?.events ?? []).filter(
    (_, i) =>
      !usedCollabIds.has(i) &&
      [
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      {visibleSteps.map((step, idx) => {
        ...
        const showDecision =
          idx === 0 || !prev || stepInstanceKey(prev) !== stepInstanceKey(step);
        ...
        const tokenStats = (() => {
          ...
          if (byInstance) {
            const total =
              (byInstance.input_tokens ?? 0) + (byInstance.output_tokens ?? 0);
            return {
              input: byInstance.input_tokens ?? 0,
              output: byInstance.output_tokens ?? 0,
              total,
            };
          }
          ...
          if (byRole) {
            const total =
              (byRole.input_tokens ?? 0) + (byRole.output_tokens ?? 0);
            return {
              input: byRole.input_tokens ?? 0,
              output: byRole.output_tokens ?? 0,
              total,
            };
          }
          return usageByRole[stepRoleKey(step)];
        })();
        const retryCount = govEvents.filter(
          (e) =>
            String(e.action || "")
              .toLowerCase()
              .includes("retry") ||
        ...
        const consultCount = collabEvents.filter(
          (e) =>
            e.type === "agent_consult_requested" ||
            e.type === "agent_consult_completed",
        ).length;
        const debateCount = collabEvents.filter(
          (e) => e.type === "debate_round",
        ).length;
            ...
            <EventCard
              ...
              onOpenFiles={
                step.files_changed.length > 0
                  ? () =>
                      onOpenFiles?.(
                        step.agent_instance || step.agent,
                        step.files_changed,
                      )
                  : undefined
              }
        ...
        <section className="narrative-queued">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {queued.map((q, idx) => {
              ...
              return (
                <span
                  key={`${q.agent_instance || q.agent}-${idx}`}
                  className={`rounded-md px-2 py-0.5 text-[10px] ${role.color} bg-[rgba(0,0,0,0.03)]`}
                >
